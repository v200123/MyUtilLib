 ...
 */
fun CharSequence.toRelativeSizeSpan(range: IntRange, scale: Float = 1.5f): CharSequence {
...
/**
 * @param range 要改变大小的文字的范围
 * @param useDip : 是否使用dp值，为false则是使用SP，即和屏幕像素相关
 */
fun CharSequence.toAbsoluteSizeSpan(range: IntRange, scale: Int = 0,useDip:Boolean = false): CharSequence {
    return SpannableString(this).apply {
        setSpan(AbsoluteSizeSpan(scale,useDip), range.first, range.last, Spannable.SPAN_INCLUSIVE_EXCLUSIVE)
    }
}
...
fun CharSequence.toClickSpan(range: IntRange, color: Int = Color.RED, isUnderlineText: Boolean = false, clickAction: ()->Unit): CharSequence {
    return SpannableString(this).apply {
        ...
        setSpan(clickableSpan, range.start, range.endInclusive, Spannable.SPAN_INCLUSIVE_INCLUSIVE)
    }
}
...
fun TextView.sizeSpan(str: String = "", range: IntRange, scale: Float = 1.5f): TextView{
    text = (if(str.isEmpty())text else str).toRelativeSizeSpan(range, scale)
    return this
}

fun TextView.appendSizeSpan(str: String = "", scale: Float = 1.5f): TextView{
    append(str.toRelativeSizeSpan(0..str.length, scale))
    return this
}
